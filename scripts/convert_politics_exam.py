...
def preprocess_table_row(line):
    ...
    while i < len(line):
        if line[i] == '`' and not in_backtick:
            in_backtick = True; result.append(line[i])
        elif line[i] == '`' and in_backtick:
            in_backtick = False; result.append(line[i])
        elif line[i] == '|' and in_backtick:
            result.append('\u0001')
        else:
...
def convert_md_to_html(lines, emoji):
    html_parts = []
    i = 0
    ...
    def flush_blockquote():
        ...
        if not blockquote_lines:
            in_blockquote = False; return
        processed = []
        idx = 0
        while idx < len(blockquote_lines):
            line = blockquote_lines[idx]
            if line.startswith('<li>'):
                processed.append('<ul>')
                while idx < len(blockquote_lines) and blockquote_lines[idx].startswith('<li>'):
                    processed.append(blockquote_lines[idx]); idx += 1
                processed.append('</ul>')
            else:
                processed.append(line); idx += 1
    ...
    while i < len(lines):
        ...
        if line.strip().startswith('```'):
            if in_code: flush_code()
            else: flush_table(); flush_list(); flush_blockquote(); in_code = True
            i += 1; continue
        ...
        if '|' in line and line.strip().startswith('|'):
            ...
            html_parts.append('<tr>' + ''.join(f'<{tag}{cls}>{process_inline(c)}</{tag}>' for c in cells) + '</tr>')
            i += 1; continue
        ...
        if line.startswith('#### '):
            ...
            id_slug = re.sub(r'[^\w\u4e00-\u9fff-]', '', text)[:30]
            html_parts.append(f'<h4 id="{id_slug}">{text}</h4>\n')
            i += 1; continue
    ...
    for m in re.finditer(r'<h2 id="([^"]*)">([^<]*)</h2>', body):
        toc_items.append(f'<li><a href="#{m.group(1)}">{m.group(2)}</a></li>')
    toc_html = '\n'.join(toc_items)

    title_match = re.search(r'^# (.+)$', lines[0] if lines else '', re.MULTILINE)
    page_title = title_match.group(1) if title_match else '常考题型'
...
<head>
    ...
    <style>
        ...
        .math-block {{ overflow-x: auto; padding: 0.5em 0; }}
...
<div class="mobile-menu" role="dialog" aria-label="导航菜单">
    ...
    <div class="page-header politics">
        ...
        <div class="breadcrumb"><a href="../index.html">首页</a> / <a href="../exam-types.html">常考题型及解法</a> / {page_title.split('常见题型')[0] if '常见题型' in page_title else page_title}</div>
    </div>
    <div class="container" id="main-content">
        <div class="toc"><h3>章节导航</h3><ul>
{toc_html}
        </ul></div>
...
# ===== 批量转换 =====
TASKS = [
    (r"C:\Users\陈鑫\Desktop\知识库\政治\题型\03-史纲常见题型.md",
     r"E:\coding\demo\docs\politics\history-exam-types.html", "📜"),
    (r"C:\Users\陈鑫\Desktop\知识库\政治\题型\04-思修常见题型.md",
     r"E:\coding\demo\docs\politics\morality-exam-types.html", "📘"),
    (r"C:\Users\陈鑫\Desktop\知识库\政治\题型\05-形势与政策常见题型.md",
     r"E:\coding\demo\docs\politics\policy-exam-types.html", "🌍"),
]

os.makedirs(r"E:\coding\demo\docs\politics", exist_ok=True)

for input_path, output_path, emoji in TASKS:
    with open(input_path, "r", encoding="utf-8") as f:
        input_lines = f.readlines()
    html = convert_md_to_html(input_lines, emoji)
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html)
    # Stats
    tables = html.count('<div class="table-wrapper"')
    code_blocks = html.count('<pre>')
    blockquotes = html.count('</blockquote>')
    print(f"✅ {os.path.basename(output_path)}: {len(input_lines)}行 → {tables}T/{code_blocks}C/{blockquotes}B")
